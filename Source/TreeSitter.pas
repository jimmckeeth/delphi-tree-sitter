...
type
  ETreeSitterException = class(Exception);
  ETreeSitterNotLoadedException = class(ETreeSitterException);
...
procedure CheckTreeSitterLoaded;
begin
  if not Assigned(ts_parser_new) then
    raise ETreeSitterNotLoadedException.Create('Tree-sitter library not loaded. Call TTreeSitterLoader.Load first.');
end;
